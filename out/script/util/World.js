define(["require", "exports", "Tiles", "util/Vector"], function (require, exports, Tiles_1, Vector_1) {
    "use strict";
    ...
    const mappedTiles = [Tiles_1.Tile.Grass, Tiles_1.Tile.Path];
    class World {
        constructor(size = Vector_1.Vector(32)) {
            this.size = size;
        ...
        clear() {
            this.tiles.fill(Tiles_1.Tile.None);
        }
        remap() {
        ...
        render(canvas) {
            for (let y = 0; y < this.size.y; y++) {
                for (let x = 0; x < this.size.x; x++) {
                    ...
                    if (tile == Tiles_1.Tile.None) {
                        continue;
                    }
                    canvas.drawSubTiles(Tiles_1.Tile[tile], Vector_1.Vector(x, y), this.getSubTiles(this.mappings[this.getTileLocation(x, y)]));
                }
            }
        }
        getTile(x, y) {
